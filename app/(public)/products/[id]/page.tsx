  ...
  return (
    <div>
      <style>{`
        ...
        .btn-steel { background: var(--fog); color: var(--ink); border: 1.5px solid var(--mist); padding: 0.68rem 1.3rem; }
        .btn-steel:hover { background: var(--mist); }

        .related-card { transition: all 0.25s; }
        .related-card:hover { transform: translateY(-4px); box-shadow: 0 12px 30px rgba(30,34,41,0.12); border-color: rgba(75,182,232,0.35) !important; }

        .rich-content { color: #4a5568; line-height: 1.8; font-size: 0.92rem; }
        .rich-content h2, .rich-content h3 { font-family: 'Barlow Condensed', sans-serif; font-weight: 700; color: var(--ink); }
      ...
      {/* ── BREADCRUMB ── */}
      <div style={{ background: 'var(--ink)', padding: '13px 0', borderBottom: '1px solid rgba(75,182,232,0.08)' }}>
        <div style={W}>
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '0.66rem', color: 'var(--slate)', letterSpacing: '0.4px', display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
            ...
            {category && (
              <><span style={{ opacity: 0.3 }}>›</span>
              <Link href={`/products?category=${category.slug}`} style={{ color: 'var(--slate)', textDecoration: 'none' }}>{category.name}</Link></>
            )}
            <span style={{ opacity: 0.3 }}>›</span>
            <span style={{ color: 'var(--brand)' }}>{product.title}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...W, paddingTop: '2.8rem', paddingBottom: '1.5rem' }}>
        <div className="detail-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3.5rem', alignItems: 'start' }}>
          ...
          {/* ── IMAGE ── */}
          <div>
            <div style={{ borderRadius: 12, overflow: 'hidden', boxShadow: '0 5px 28px rgba(30,34,41,0.12)', border: '1px solid rgba(75,182,232,0.12)', background: 'linear-gradient(145deg, var(--ink), var(--steel))', position: 'relative', minHeight: 300 }}>
              <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(75,182,232,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(75,182,232,0.04) 1px,transparent 1px)', backgroundSize: '22px 22px', zIndex: 0 }} />
              {product.image ? (
                <img src={product.image} alt={product.title} style={{ width: '100%', height: '100%', objectFit: 'contain', objectPosition: 'center', display: 'block', position: 'relative', zIndex: 1, maxHeight: 400, background: 'rgba(255,255,255,0.02)' }} />
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 300, position: 'relative', zIndex: 1 }}>
                  ...
                  <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '0.56rem', letterSpacing: '2.5px', color: 'rgba(75,182,232,0.3)', marginTop: '0.6rem' }}>FLOWMAN ENGINEERS</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            {/* Category badge */}
            {category && (
              <div style={{ display: 'inline-block', background: 'rgba(75,182,232,0.10)', color: 'var(--cobalt)', border: '1px solid rgba(75,182,232,0.25)', padding: '3px 10px', borderRadius: 2, fontSize: '0.6rem', fontWeight: 700, letterSpacing: '2px', fontFamily: 'DM Mono, monospace', textTransform: 'uppercase', marginBottom: '0.85rem' }}>{category.name}</div>
            )}
            ...
            <div style={{ background: 'var(--fog)', border: '1px solid var(--mist)', borderRadius: 8, padding: '1.1rem', marginBottom: '1.4rem' }}>
              ...
              <div className="spec-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.85rem' }}>
                {[
                  ...
                  ['Manufacturer', 'Flowman Engineers'],
            ...
            <div style={{ display: 'flex', gap: '0.7rem', flexWrap: 'wrap' }}>
              ...
              <Link href="/contact" className="btn btn-steel">Contact Us</Link>
              <a
        ...
        <div style={{ marginTop: '3rem', borderTop: '1px solid var(--mist)', paddingTop: '2.5rem' }}>
          <div className="desc-flex" style={{ display: 'flex', gap: '2.8rem', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 2, minWidth: 0 }}>
              <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '0.58rem', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--brand)', marginBottom: '0.45rem' }}>Technical Details</div>
            ...
            <div className="desc-sidebar" style={{ flex: '0 0 250px', position: 'sticky', top: 85 }}>
              <div style={{ background: 'var(--ink)', borderRadius: 10, padding: '1.4rem', border: '1px solid rgba(75,182,232,0.15)', borderTop: '3px solid var(--brand)' }}>
                <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '0.56rem', letterSpacing: '3px', color: 'var(--brand)', marginBottom: '0.6rem' }}>GET A QUOTE</div>
                ...
                <div style={{ marginTop: '1.1rem', paddingTop: '1rem', borderTop: '1px solid rgba(75,182,232,0.12)' }}>
                  ...
                  <a href="tel:+919725944834" style={{ fontFamily: 'DM Mono, monospace', fontSize: '0.76rem', color: 'var(--brand)', textDecoration: 'none', display: 'block', marginTop: '0.35rem' }}>+91-9725944834</a>
                </div>
              </div>
            </div>
          </div>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: '3rem', borderTop: '1px solid var(--mist)', paddingTop: '2.5rem', paddingBottom: '2.5rem' }}>
            <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '0.58rem', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--brand)', marginBottom: '0.45rem' }}>More Like This</div>
            ...
            <div className="related-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.2rem' }}>
              {related.map(p => (
                <Link key={p.id} href={`/products/${p.id}`} style={{ textDecoration: 'none' }}>
                  <div className="related-card" style={{ background: 'white', border: '1px solid var(--mist)', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 6px rgba(30,34,41,0.05)' }}>
                    <div style={{ height: 145, background: 'linear-gradient(145deg, var(--ink), var(--steel))', position: 'relative', overflow: 'hidden' }}>
                      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(75,182,232,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(75,182,232,0.04) 1px,transparent 1px)', backgroundSize: '18px 18px' }} />
                      {p.image
                        ? <img src={p.image} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', inset: 0 }} />
                        : <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.7rem', opacity: 0.22, color: 'white' }}>⚙</div>
                      }
                    </div>
